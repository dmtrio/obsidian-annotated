...
- **D3 — Echo-loop prevention (the frontmatter analog of the comment self-exclusion).** The agent writes receipts, and a naive watcher would see its own `reviewed` write as a fresh event. Two composable guards, mirroring how the comment watcher excludes its own identity:
  ...
  - **Invariant — a `completedTag` must be disjoint from every trigger root.** This is what makes a receipt mean *done*: if the receipt equals its trigger, the two states are the same string, so the note can never read as finished, crash recovery (fresh ledger) re-announces and re-runs completed work, and humans/Dataview can't distinguish requested from done. The past-tense default satisfies this for free (`reviewed` ≠ `review`). The one failure mode is a verb whose past tense *equals* its present (`proofread` → `proofread`): such an action **must** declare a distinct receipt (e.g. `proofed`), and the skill **validates this at load** — an action whose `completedTag` collides with any trigger root is rejected. The collision is the signal that a distinct receipt is required, not an edge case to tolerate.
...
4. **Skill: externalized action files + handling rules (D1a).** Extend `watch-vault` (or a paired skill): `SKILL.md` carries the stable rules (request→work→receipt loop, `blocked`+comment on unknown, echo-guard, `active`-mode injection with its D6 cap) and the logic that reads `actions/*.yml` → derives `WATCH_TRIGGERS` + the receipt exclude-set → arms the monitor. Ship the starter set as `actions/{review,summarize,active}.yml`. The agent derives receipts from `completedTag` (or past tense if omitted) and refuses to guess unknown values. Validate the D3 invariant at load: reject any action whose `completedTag` (resolved, with `{arg}` stripped) collides with a trigger root. *Accept: dropping in one new `actions/<name>.yml` adds a working workflow with no `SKILL.md` change; the monitor receives the trigger list without parsing YAML; a colliding-receipt action is rejected with a clear message; dry-run against a seeded note set.*
